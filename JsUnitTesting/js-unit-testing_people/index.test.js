...
test('getPeopleNamesOlderThan() returns an array of full names of people older than a certain age, e.g. "Luke Skywalker"', () => {
  const result = getPeopleNamesOlderThan(people, 30);
  expect(result).toEqual(["Max Mustermann", "Erika Musterfrau"]);
});

test("getPeopleByLastName() returns an array of people with a certain lastName", () => {
